...
## ツイートしてくれるRスクリプトを生成
echo $dir
filehame -lLABEL $HOME/dragons_sokuhou_template.R $dir/tweet.txt > ~/dragons_sokuhou.R
echo "filehame OK" >> $HOME/Dropbox/cron.log.txt

set +e
label=$(cat ~/dragons_sokuhou.R | grep  "LABEL")
[ "$label" = "" ] || exit 1

## ツイートします
$(which R) -q --slave --vanilla -f ~/dragons_sokuhou.R
