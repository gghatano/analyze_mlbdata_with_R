...
## ツイートしてくれるRスクリプトを生成
echo $dir
filehame -lLABEL $HOME/tigers_sokuhou_template.R $dir/tweet.txt > ~/tigers_sokuhou.R
echo "filehame OK" >> $HOME/Dropbox/cron.log.txt

set +e
label=$(cat ~/tigers_sokuhou.R | grep  "LABEL")
[ "$label" = "" ] || exit 1

## ツイートします
$(which R) -q --slave --vanilla -f ~/tigers_sokuhou.R
